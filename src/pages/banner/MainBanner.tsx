...
import { useBannerListQuery } from "services/modules/banner";
...
export default function MainBanner(): React.ReactElement {
  ...
  const handleCreateBanner = (): void => {
    void push("/banner/main-banner/createBanner");
  };

  const handleEditBanner = (id: string): void => {
    void push(`/banner/main-banner/editBanner?id=${id}`);
  };
  ...
  const handleUpdateStatusBanner = async (
    ...
    try {
      const statusUpdated = { is_active: !status };
      // const response = await updateStatusBanner(id.id, statusUpdated);

      // console.log('Status Banner berhasil diubah:', response);
      // const newData = await getBanners(searchParams);
      // setDataBanners(newData);
      setWarningMessage("");
      // }
    } catch (error: any) {
      setIsWarningPopupOpen(true);
      if (error.message === "Cannot be more than 10") {
        setWarningMessage("Maximal Banner Active is 10");
      } else if (error.message === "Cannot be less than 1") {
      ...
      console.error("Terjadi kesalahan saat membuat banner:", error);
    }
  };
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center justify-between gap-4 ml-4">
              ...
              <button
                ...
                className="flex flex-row  items-center justify-center gap-x-1.5 rounded-full px-6 py-2 bg-seeds-button-green text-white"
              >
                Create Banner
                <ChevronDownIcon
