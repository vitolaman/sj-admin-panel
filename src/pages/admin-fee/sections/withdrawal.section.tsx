...
import { useGetWithdrawalListQuery } from "services/modules/admin-fee";
import { Button, Modal } from "react-daisyui";
import CInput from "components/input";
import { Controller } from "react-hook-form";
import useUpdateWithdrawForm from "hooks/admin-fee/useUpdateWithdrawForm";

const WithdrawalSection = ({ filter }: { filter?: AdminFeeFilterI }) => {
  const { data, isLoading, refetch } = useGetWithdrawalListQuery(undefined);
  ...
  } = useUpdateWithdrawForm();
  const isPromoActive = watch("is_promo_active");

  useEffect(() => {
    if (updateWithdrawFeeState.isSuccess) {
      setModal({ show: false });
      refetch();
    }
  }, [updateWithdrawFeeState.isSuccess]);
          ...
          >
            <div className="border-r border-[#9B9B9B] p-4 col-span-2 flex flex-col gap-4">
              ...
              <CInput
                disabled={true}
                value={
                  selectedData?.payment_type === "bank" ? "Bank" : "E-Wallet"
                }
              />
              <label
                htmlFor="merchant"
