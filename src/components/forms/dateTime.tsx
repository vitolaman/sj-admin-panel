...
interface Props {
  label?: string;
  name: string;
  onChange: (str: string | moment.Moment) => void;
  value?: string;
  placeholder?: string;
  timeFormat?: string;
  dateFormat?: string;
...
export function Date({
  ...
  return (
    <>
      {!isEmpty(label) && (
        <label
          className={`block leading-6 text-[#262626] font-semibold mb-2${
            isRequired ? " required" : ""
          }`}
        >
          {label}
        ...
        <Datetime
          ...
          inputProps={{
            ...
            className: `block w-full border-b ${
              !isEmpty(isError) ? "border-[#DD2525]" : "border-[#BDBDBD]"
            } py-3 pl-7 text-[#262626] h-11 leading-4 placeholder:text-[#BDBDBD] focus:outline-0 disabled:bg-[#E9E9E9]`,
          }}
        />
        <div className="absolute inset-y-0 left-0 flex items-center pointer-events-none">
          <CalendarIcon className="w-5 h-5 text-[#323232]" />
        </div>
      </div>
      {!isEmpty(isError) && (
