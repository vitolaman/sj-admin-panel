  ...
  return (
    ...
    >
      {({ open }) => (
        <>
          {!isEmpty(label) && (
            <Listbox.Label className="block leading-6 text-[#262626] font-semibold mb-2">
              {label}
            </Listbox.Label>
          )}
          <div className="relative">
            <Listbox.Button
              className={classNames(
                "relative w-full cursor-default rounded-full border border-[#BDBDBD] p-3 text-[#262626] h-11 leading-4 placeholder:text-[#BDBDBD] focus:outline-0 disabled:bg-[#E9E9E9]",
                !isEmpty(isError) ? "border-[#DD2525]" : ""
              )}
            >
              <span className="flex items-center">
                <div>{value?.icon !== undefined && value.icon()}</div>
                <span className="ml-2 block text-[#262626] truncate">
                  {value?.label !== undefined ? value.label : placeholder}
                </span>
              </span>
              <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                <ChevronDownIcon className="w-5 h-5 text-[#323232]" />
              </div>
            </Listbox.Button>
            {!isEmpty(isError) && (
              <div className="flex items-center gap-2 mt-2">
                ...
                <p className="text-xs text-[#DA2D1F]">{isError}</p>
              </div>
            )}
            ...
            <Transition
              ...
              leaveTo="opacity-0"
            >
              <Listbox.Options className="absolute z-50 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                {options?.map((option) => (
                  <Listbox.Option
                    key={option.key}
                    className={({ active }) =>
                      classNames(
                        active ? "bg-indigo-600 text-white" : "text-[#262626]",
                        "relative cursor-default select-none",
                        isBiggerSize ? "py-3 px-2" : "p-2"
                      )
                    }
                    value={option}
                  >
